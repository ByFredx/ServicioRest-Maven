...
@Path("/usuarios")
@Api(value="usuarios", description="Ejemplo de documentacion con SWAGGER service")
@Produces(MediaType.APPLICATION_JSON)
public class UsuarioController {

	private static final List<Usuario> USUARIOS = new ArrayList<Usuario>();
	static {
		USUARIOS.add(new Usuario(1L, "Fredy", "Huachaca", 28));
		USUARIOS.add(new Usuario(2L, "Marco", "Huachaca", 26));
		USUARIOS.add(new Usuario(3L, "Alex", "Huachaca", 24));
		USUARIOS.add(new Usuario(4L, "Frank", "Huachaca", 2));
	}
	...
	public Response registrar(
			@ApiParam(value = "Datos del nuevo usuario", 
					  required = true) Usuario nuevousuario) {
	...
	public Response modificar(
			 @ApiParam(value = "ID del usuario a modificar", allowableValues = "range[1," + Integer.MAX_VALUE + "]", required = true)
			 @PathParam("id") Long id, 
			 @ApiParam(value = "Datos del usuario a modificar", required = true) 
	...
	public Response eliminar(
			@ApiParam(value = "ID del usuario a eliminar", 
